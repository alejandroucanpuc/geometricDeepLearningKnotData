...
class Crossing():
    """
    Esta clase representa un cruce en un diagrama planar de un nudo.
    Un cruce conecta cuatro caminos (paths) y tiene varias operaciones asociadas.
    """
...
class Knot():
    """
    Esta clase representa un nudo utilizando su diagrama planar.
    Un nudo está compuesto por múltiples cruces (Crossing) y tiene
    varias operaciones asociadas.
    """
    def __init__(self,k=[]):
    ...
    def facingPaths(self,path,rotation:int=1):
        """
        Returns the paths facing the given path in order of encounter when walking along the knot.
        The rotation parameter determines the direction of walking (1 for clockwise, -1 for counterclockwise).
        """
        # caching
    ...
    def relableKnot(self):
        """
        Relables the knot such that paths are numbered from 0 to n-1
